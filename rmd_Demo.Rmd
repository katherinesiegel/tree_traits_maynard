    ...
    mutate(longitude = ifelse(longitude == " 83h 22m",
         ...
         latitude = ifelse(latitude == "35.-0213",
                           ...
                           ifelse(latitude == "47.-77262",
                                  "47.77262",
                                  latitude)),
         latitude = ifelse(orig_ref == "Pierce S., Brusa G., Vagge I., Cerabolini B.E.L. (2013) Allocating CSR plant functional types: the use of leaf economics and size traits to classify woody and herbaceous vascular plants. Functional Ecology, 27(4): 1002-1010",
                           "45.8",
                           latitude),
         longitude = ifelse(orig_ref == "Pierce S., Brusa G., Vagge I., Cerabolini B.E.L. (2013) Allocating CSR plant functional types: the use of leaf economics and size traits to classify woody and herbaceous vascular plants. Functional Ecology, 27(4): 1002-1010",
                            "8.88333",
                            longitude))
...
### trait units
```{r}
### which units are there for each trait?
trait_units <- trait_dat_coords %>%
  group_by(trait_name_short, unit) %>%
  summarise(n_rows = n())

### fix units
trait_dat_fix <- trait_dat_coords %>%
  
  ### drop ldmc_fresh with unit = SLA cm2/g
  filter(!(trait_name_short == "ldmc_fresh" & unit == "SLA cm2/g"))
```
